...
const Chat = () => {
  ...
  // For stylists, the customer is participant[0]
  const contactInfo =
    conversation?.participant_details?.[0] || temporaryContact;
  const contactName =
    conversation?.participant_details?.[1]?.name?.split(" ")[0] ||
    temporaryContact?.name?.split(" ")[0] ||
    "Chat";
  const contactAvatar =
    conversation?.participant_details?.[0]?.profile_image_url ||
        ...
        >
          ...
          >
            <Pressable
              ...
              onPress={() => router.replace("/stylist/messages")}
            >
              ...
              onPress={() => {
                if (!user?.sub_account) {
                  Alert.alert(
                    ...
                    "Please set up your bank account before creating an order."
                  );
                  router.push("/stylist/set-account");
                } else {
                  router.push({
                    pathname: "/stylist/orders/create",
                    params: {
                      conversation_id: id,
                      customer_id:
                        conversation?.participants_map?.customer ||
                        conversation?.participants[0] ||
                        "",
                      stylist_id:
                        conversation?.participants_map?.stylist ||
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            <Text style={{ fontSize: 10, color: "black", textAlign: "center" }}>
              Make sure to create an order after agreeing with the customer on
              price, delivery date, delivery location and any other agreements.
            </Text>
          </View>
          ...
          >
            ...
            {otherUserTyping && (
              ...
              >
                <Avatar
                  uri={conversation?.participant_details?.[0].profile_image_url}
                  width={24}
                  height={24}
                />
                <View
