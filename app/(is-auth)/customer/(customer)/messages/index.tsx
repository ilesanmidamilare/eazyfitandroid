...
const MessagesPage = () => {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      // Search by the other person's name
      const participantName =
        conversation.participant_details?.[1]?.name?.toLowerCase() || "";
  ...
  useEffect(() => {
    const handleMessage = (msg: any) => {
      ...
      console.log("Received message in conversations list:", msg.type);
    };
  ...
  const renderConvo = ({ item }: { item: ChatData }) => {
    ...
    return (
      ...
      >
        <Avatar uri={item?.participant_details?.[1].profile_image_url} />
        ...
        <View style={{ flex: 1, gap: 5 }}>
          ...
          >
            {item?.participant_details?.[1].name}
          </Heading>
