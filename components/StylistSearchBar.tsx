...
const StylistSearchBar: React.FC<StyleSearchBarProps> = ({
  text,
  setText,
  checkedOptions,
  setCheckedOptions,
  ...
  const toggleCheck = (option: string) => {
    if (setCheckedOptions && checkedOptions) {
      if (checkedOptions.includes(option)) {
        setCheckedOptions(
          checkedOptions.filter((item: string) => item !== option)
        );
      } else {
  ...
  return (
    <View style={{ marginBlock: 15 }}>
      <View style={styles.searchWrapper}>
        ...
        <Pressable onPress={() => setVisible && setVisible(!visible)}>
